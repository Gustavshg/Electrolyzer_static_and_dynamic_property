...
def compare_polar_and_original(V_res,voltage):
    ...
    plt.title('recovered voltage-Harmonic average by cooling ratio SGD rs/t step by step')
...
'''原始方程参数'''
ar1=tf.Variable(initial_value=7.3860174e-05)
ar2=tf.Variable(initial_value=4.571728e-06)
ar3=tf.Variable(initial_value=-3.6577415e-08)
as1=tf.Variable(initial_value=0.06539263)
as2=tf.Variable(initial_value=0.0008087277)
as3=tf.Variable(initial_value=-2.0567513e-05)
at1=tf.Variable(initial_value=0.1902913)
at2=tf.Variable(initial_value=5.761193)
at3=tf.Variable(initial_value=1507.3917)
br1=tf.Variable(initial_value=6.288664e-05)
br2=tf.Variable(initial_value=9.1818954e-08)
br3=tf.Variable(initial_value=-5.119298e-09)
bs1=tf.Variable(initial_value=0.06427209)
bs2=tf.Variable(initial_value=0.00041437536)
bs3=tf.Variable(initial_value=4.960586e-06)
bt1=tf.Variable(initial_value=0.19109061)
bt2=tf.Variable(initial_value=-31.187824)
bt3=tf.Variable(initial_value=1456.7318)
# Overall loss =  0.3202179
...
'''优化器设置'''
optimizer = tf.keras.optimizers.Adam(learning_rate = 1E-11)
    ...
    for i in range(num_epoch):
        ...
        while start<len(voltage):
            with tf.GradientTape() as tape:
                ...
                V_pred = Ures + 1 / (lam / V_out + (1-lam) / V_in)
                T_ave = 1 / (lam / T_out + (1 - lam) / T_in)
                '''取批次进行训练'''
...
if pred == 1:
    ...
    V_in_re = current_density * (br1 + br2 * T_in + br3 * T_in ** 2) + (bs1 + bs2 * T_in + bs3 * T_in ** 2) \
           * tf.math.log(current_density * (bt1 + bt2 / T_in + bt3 / T_in ** 2) + 1)
    V_recover = Ures + 1 / (lam / V_out + (1-lam) / V_in)
