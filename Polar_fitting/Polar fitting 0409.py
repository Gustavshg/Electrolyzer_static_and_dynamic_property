...
def compare_polar_and_original(V_res,voltage):
    ...
    plt.title('recovered voltage-Temp average by cooling ratio SGD rs/t step by step')
...
T_ave  = (T_out + T_in)/2
plt.plot(T_ave)
T_ave1 = tf.square(lam * T_out ** 2 + (1-lam) * T_in ** 2)
plt.plot(T_ave1)
plt.legend(['Normal mean','Geometry mean'])
plt.show()

Ures = []
for i in range(len(T_out)):
    Ures.append(Vres((T_out + T_in)/2))
