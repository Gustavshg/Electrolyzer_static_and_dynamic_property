...
def compare_polar_and_original(V_res,voltage):
    ...
    plt.title('recovered voltage-SGD rs/t step by step')
...
T_in = np.array(df['T_in'])/100
T_out = np.array(df['T_out'])/100
LyeFlow = np.array(df['LyeFlow'])/1.6
voltage /= 34*2.5
current_density = current/0.425/4000

voltage_ori = np.array(df['Voltage'])
current_ori = np.array(df['Current'])
T_in_ori = np.array(df['T_in'])/100
T_out_ori = np.array(df['T_out'])/100
LyeFlow_ori = np.array(df['LyeFlow'])/1.6
current_density_ori = current_ori/0.425/4000
voltage_ori /=34*2.5
...
'''原始方程参数'''
r1=tf.Variable(initial_value=1.1048578)
r2=tf.Variable(initial_value=-1.0415688)
r3=tf.Variable(initial_value=0.036688816)
s1=tf.Variable(initial_value=-0.14951457)
s2=tf.Variable(initial_value=0.042033684)
s3=tf.Variable(initial_value=0.007578711)
t1=tf.Variable(initial_value=-1268.7379)
t2=tf.Variable(initial_value=3818.4624)
t3=tf.Variable(initial_value=-1412.0209)
...
# optimizer = tf.keras.optimizers.SGD(learning_rate = 1E8)
...
pred = 1
if pred == 1:
    V_recover = Ures + current_density * (r1 + r2 * T_ave + r3 * T_ave ** 2) + (s1 + s2 * T_ave + s3 * T_ave ** 2) \
             * tf.math.log(current_density * (t1 + t2 / T_ave + t3 / T_ave ** 2) + 1)
