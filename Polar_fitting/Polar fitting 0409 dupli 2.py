...
def compare_polar_and_original(V_res,voltage):
    ...
    plt.title('recovered voltage-SGD all para')
...
'''原始方程参数'''
r1=tf.Variable(initial_value=1.0820339)
r2=tf.Variable(initial_value=-1.0445125)
r3=tf.Variable(initial_value=0.04164544)
s1=tf.Variable(initial_value=-0.12694563)
s2=tf.Variable(initial_value=0.029924586)
s3=tf.Variable(initial_value=-0.016782567)
t1=tf.Variable(initial_value=-2200.1274)
t2=tf.Variable(initial_value=3317.766)
t3=tf.Variable(initial_value=-650.9243)
variables = [r1,r2,r3,s1,s2,s3,t1,t2,t3]
'''给入口温度乘上碱液流量直接进行拟合'''
T_ave  = (T_out + T_in)/2
