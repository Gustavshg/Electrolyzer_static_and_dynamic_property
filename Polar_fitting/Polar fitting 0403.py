...
def compare_polar_and_original(V_res,voltage):
    ...
    plt.title('recovered voltage-Adam r 20000')
...
'''原始方程参数'''
r1=tf.Variable(initial_value=3.800538e-05)
r2=tf.Variable(initial_value=8.689829e-06)
r3=tf.Variable(initial_value=6.446653e-07)
s1=tf.Variable(initial_value=406.08545)
s2=tf.Variable(initial_value=2408.9333)
s3=tf.Variable(initial_value=-73.682076)
t1=tf.Variable(initial_value=7.402238e-08)
t2=tf.Variable(initial_value=-7.942174e-06)
t3=tf.Variable(initial_value=0.00029215112)
...
if train == 1:
    ...
    for v in range(len(variables)):
        print(varname(variables[v]) +' gradient ='  + str(grads[v].numpy()) )
