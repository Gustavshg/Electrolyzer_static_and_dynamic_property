...
def compare_polar_and_original(V_res,voltage):
    ...
    plt.title('recovered voltage-Geometric average by cooling ratio SGD rs/t step by step')
...
'''原始方程参数'''
ar1=tf.Variable(initial_value=7.57808e-05)
ar2=tf.Variable(initial_value=7.416939e-06)
ar3=tf.Variable(initial_value=-5.2104777e-08)
as1=tf.Variable(initial_value=0.0645596)
as2=tf.Variable(initial_value=0.00032138845)
as3=tf.Variable(initial_value=-2.5627454e-05)
at1=tf.Variable(initial_value=0.17952952)
at2=tf.Variable(initial_value=-9.607382)
at3=tf.Variable(initial_value=3679.3052)
br1=tf.Variable(initial_value=6.1260966e-05)
br2=tf.Variable(initial_value=3.3049207e-07)
br3=tf.Variable(initial_value=-1.3716852e-08)
bs1=tf.Variable(initial_value=0.06504009)
bs2=tf.Variable(initial_value=0.00013342549)
bs3=tf.Variable(initial_value=1.3084227e-05)
bt1=tf.Variable(initial_value=0.1902)
bt2=tf.Variable(initial_value=-26.843899)
bt3=tf.Variable(initial_value=1201.5348)
# Overall loss =  0.28582945
...
if pred == 1:
    ...
    V_in_re = current_density * (br1 + br2 * T_in + br3 * T_in ** 2) + (bs1 + bs2 * T_in + bs3 * T_in ** 2) \
           * tf.math.log(current_density * (bt1 + bt2 / T_in + bt3 / T_in ** 2) + 1)
    V_recover = Ures + tf.sqrt(lam * V_out ** 2 + (1-lam) * V_in ** 2)
