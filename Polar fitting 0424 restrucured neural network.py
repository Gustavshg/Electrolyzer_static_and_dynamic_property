...
class DataLoader():
    ...
    def __init__(self):
        ...
        current_density = np.array(df['Current']).reshape(len(df), 1)
        ...
        current_density /= 0.425 * 4000
        voltage /= 34
        T_out /= 100
        T_in /= 100

        self.all_data_x = np.concatenate((T_out, T_in, current_density, lam), axis=1)
...
learning_rate = 1E-3
batch_size = 50
storage_file = 'Neural Networks/Polarization/Trial/checkpoint-0423-8.ckpt'
...
for epoch_id in range(num_epochs):
    ...
    if save == 1:
        if epoch_id%100 == 50:
            model.save(storage_file)


if save == 1:
    # model.save('Neural Networks/Polarization/Standard-0425/standard-model.ckpt')
    model.save(storage_file)
