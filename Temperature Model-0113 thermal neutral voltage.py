...
SelectedFiles = ["20s/Original/TJ-20211014.csv"]
...
for file in SelectedFiles:
    ...
    if Ignore == 0 or  FlagIgnore ==0:
        ...
        df['E'] = (df['电解电压'] -df['Vtn']*34)  * df['电解电流']/1000
        df['RadTemp'] = ((df['OriTemp'] +273.15 )**4  -(df['AmbT'] +273.15 )**4 )  #热辐射
        ...
        index = 35
        inputs = [df['E'][index], df['RadTemp'][index], df['HeatLyeIn'][index], df['HeatLyeOut'][index], df['DeltaTemp'][index]]
        #print(inputs )
        energy_form= [inputs[0]  , inputs[1]*-3.19807956e-13/9.76913600e-04 ,inputs[2]*5.66544937e-04/9.76913600e-04 ,inputs[3]*-6.41418079e-04/9.76913600e-04]
        #print(energy_form)
        #print(20*(sum(energy_form))*9.76913600e-04,inputs[4])
        ...
        if len(SelectedFiles) ==12:
            ...
            if FlagIgnore == 0:
                ...
                yy=clf.predict(X)
                compare_DeltaandTemp(df, yy, file, plt)
                print(file, "Error: ", err, "Coef, ", clf.coef_)
                counter = counter + 1
                if orifile =="TJ-20211001.csv" or orifile =="TJ-20211007.csv" or orifile =="TJ-20211014.csv" or orifile =="TJ-20211029.csv":
                    data2figure0311 = []
                    data2figure0311.append(df['OriTemp'])
                    data2figure0311.append(df['DeltaTemp'])
                    data2figure0311.append(retrieve_temp(yy,df['OriTemp'][0]))
                    data2figure0311.append(yy)

                    data2figure0311 = np.array((data2figure0311))
                    pandas.DataFrame(data2figure0311.T).to_csv("20s/ToFigure0311/"+orifile)
                    print(len(data2figure0311),len(data2figure0311[0]))
        else:
        ...
        coef = coef.append([[file,err,clf.coef_[0],clf.coef_[1],clf.coef_[2],clf.coef_[3]]])
...
coef = coef.append([[SourceFolder,err,clf.coef_[0][0],clf.coef_[0][1],clf.coef_[0][2],clf.coef_[0][3]]])

coef.to_csv("Coefficient-0113.csv")
